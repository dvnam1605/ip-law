#!/usr/bin/env python3
"""
Verdict Pipeline — Chunk + Ingest to Neo4j in one run.

Combines:
  1. chunking/verdict_chunker.py       (TXT → JSON with embeddings)
  2. utils/verdict_neo4j_ingest.py     (JSON → Neo4j)

Usage:
  python scripts/run_verdict_pipeline.py                   # full pipeline
  python scripts/run_verdict_pipeline.py --skip-ingest     # chỉ chunk
  python scripts/run_verdict_pipeline.py --skip-chunk      # chỉ ingest
"""
import argparse
import json
import os
...
# ── Paths ──────────────────────────────────────────────
TXT_FOLDER = PROJECT_ROOT / "output-ban-an"
CHUNKS_JSON = PROJECT_ROOT / "chunking" / "verdict_chunks.json"
EMBEDDINGS_JSON = PROJECT_ROOT / "chunking" / "verdict_chunks_with_embeddings.json"
...
def run_chunking():
    ...
    print("=" * 60)
    print("⚖️  STEP 1: VERDICT CHUNKING")
    print("=" * 60)
    ...
    export_with_embeddings(data, str(EMBEDDINGS_JSON))
...
def run_ingest():
    ...
    from utils.verdict_neo4j_ingest import (
        ...
        setup_vector_index, ingest_embeddings, print_stats,
    )

    print("\n" + "=" * 60)
    print("🗄️  STEP 2: NEO4J VERDICT INGEST")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Verdict Pipeline: Chunk → Ingest Neo4j"
    )
